...
-> 0
    ...
    public static void main(String[] args) {
        try (BufferedReader br = new BufferedReader(new InputStreamReader(System.in))) {
            ...
            int[] current = new int[4]; //현재 윈도우 내 문자 개수
            int answer = 0;
            // 초기 윈도우 셋팅
            for (int i = 0; i < P; i++) {
                addChar(current, arr[i]);
            }
            ...
            for (int i = P; i < S; i++) {
                addChar(current, arr[i]);    // 새 문자 추가
                remove(current, arr[i - P]); //빠져나가는 문자 제거
    ...
    private static void addChar(int[] current, char ch) {
        switch (ch) {
            case 'A':
                current[0]++;
                break;
    ...
    private static void remove(int[] current, char ch) {
        switch (ch) {
            case 'A':
                current[0]--;
                break;
